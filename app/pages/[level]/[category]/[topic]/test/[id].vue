        ...
        <UCard>
          <div class="space-y-6">
            <div class="text-center">
              <UIcon name="i-heroicons-clipboard-document-check" class="mx-auto text-6xl text-primary-600" />
              <h1 class="mt-4 text-3xl font-bold text-gray-900 dark:text-white">
                {{ currentTest.title }}
              </h1>
              <p v-if="currentTest.description" class="mt-2 text-gray-600 dark:text-gray-400">
            ...
            <div class="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-800">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Liczba pytań</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ totalQuestions }}</span>
              </div>
              <div v-if="currentTest.time_limit" class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Limit czasu</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ currentTest.time_limit }} minut</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Próg zaliczenia</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ currentTest.passing_score }}%</span>
              </div>
            </div>
      ...
      <div v-else-if="testStarted && !testResult" class="mx-auto max-w-4xl">
        ...
        <UCard class="mb-6">
          <div class="flex items-center justify-between gap-4">
            ...
            <div v-if="currentTest?.time_limit" class="flex items-center gap-2 text-gray-700 dark:text-gray-300">
          ...
          >
            <div class="space-y-4">
              ...
              <div class="flex items-start gap-3">
                <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary-100 text-sm font-bold text-primary-700 dark:bg-primary-900 dark:text-primary-300">
                  {{ index + 1 }}
                </div>
                <div class="flex-1">
              ...
              <div class="space-y-2 pl-11">
                ...
                >
                  <input
                    ...
                    @change="handleAnswerSelect(tq.question.id, answer.id, answer.label)"
                    class="h-4 w-4 text-primary-600 focus:ring-primary-500"
                  />
                  <span class="font-medium text-gray-700 dark:text-gray-300">{{ answer.label }}.</span>
          ...
          <div class="space-y-6">
            ...
            <div class="text-center">
              <div class="mx-auto flex h-32 w-32 items-center justify-center rounded-full"
                :class="testResult.score >= (currentTest?.passing_score || 60)
              ...
              >
                <div>
                  ...
                  >
                    {{ testResult.score >= (currentTest?.passing_score || 60) ? 'Zaliczono' : 'Niezaliczono' }}
                  </div>
                </div>
              </div>
            ...
            <div class="space-y-3 border-t border-gray-200 pt-6 dark:border-gray-800">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Poprawne odpowiedzi</span>
                <span class="text-sm font-bold text-green-600 dark:text-green-400">
                  {{ testResult.correctAnswers }} / {{ testResult.totalQuestions }}
                </span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Niepoprawne odpowiedzi</span>
                <span class="text-sm font-bold text-red-600 dark:text-red-400">
                  {{ testResult.incorrectAnswers }}
                </span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Czas trwania</span>
...
const { currentTest, testResult, answeredCount, totalQuestions, progress, timeElapsed } = storeToRefs(testStore)
...
function handleAnswerSelect(questionId: string, answerId: string, answerLabel: string) {
